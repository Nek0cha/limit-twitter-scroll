...
強制的にスクロールを止める・時間制限をかけるのではなく、「通行料ゲージ」というメタファーを使い、スクロールするたびにゲージが溜まっていく → 満タンになるとスクロールをロックして確認を挟む → 「続ける」を選ぶと次からゲージが溜まりにくくなる（＝だんだんスクロールが重くなる）、という体験を作ります。
...
- 拡張機能アイコンをワンクリックすると設定画面が開き、初期閾値・減少率・「休憩する」時にぼかしてロックするかをカスタマイズ可能 (初期閾値は8000くらいがちょうどいいです。)
